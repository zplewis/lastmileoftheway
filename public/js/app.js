...
__webpack_require__(/*! ./utilities */ "./resources/js/utilities.js");
...
(function (utilities, $, window, document, axios, undefined) {
  ...
  var serviceTypeSelectors = document.querySelectorAll('.service-type-selector input[type="radio"]'); // Listener function that responds to the radio button click
  ...
  if (scriptureSelects) {
    ...
    try {
      for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
        var scriptureSelect = _step3.value;
  ...
  if (dateService) {
    ...
    if (testInput.value !== value && questionContentDiv) {
      console.log('input[type="datetime-local"] is not supported by this browser. Showing replacements...'); // hide the dateService element

      dateService.classList.add('d-none'); // show the hidden date and time elements
...
(function (utilities, $, window, document, axios, undefined) {
  // This allows for javascript that should work pretty consistently across browsers and platforms.
  'use strict'; // Register the CSRF token as a common header for jQuery and Axios so that
  ...
  utilities.isNullOrEmptySpace = function (x) {
    return utilities.isNullOrEmpty(x) || typeof x.trim === 'function' && utilities.isNullOrEmpty(x.trim().replace(/ /g, ''));
  };
  /**
  ...
  utilities.hasProperty = function (obj, prop) {
    return utilities.isNullOrEmpty(obj) === false && utilities.isNullOrEmptySpace(prop) === false && (Object.prototype.hasOwnProperty.call(obj, prop) || obj.hasOwnProperty(prop) || obj.prop !== undefined || 'undefined' !== typeof obj[prop]);
  };
  /**
  ...
  utilities.isEmptyObject = function (obj) {
    ...
    if (Object && utilities.hasProperty(Object, 'getOwnPropertyNames')) {
      ...
      console.log('isEmptyObject(): getOwnPropertyNames revealed ' + length + ' properties in this object.');
  ...
  utilities.getUrlVar = function (key) {
    var queryString = utilities.getQueryString(); // empty cases

    if (utilities.isNullOrEmptySpace(key) || utilities.isNullOrEmptySpace(queryString)) {
      return '';
    }
  ...
  utilities.getQueryString = function () {
    return !utilities.isNullOrEmpty(window) && !utilities.isNullOrEmpty(window.location) && !utilities.isNullOrEmptySpace(window.location.search) ? window.location.search : '';
  };
  /**
  ...
  */
  ...
  utilities.findParent = function (child, tag, className) {
    ...
    var hasClassName = utilities.isNullOrEmptySpace(className) === false; // empty case: missing child or both tag and className

    if (utilities.isNullOrEmptySpace(child) || hasTag === false && hasClassName === false) {
    ...
    while (child.parentNode) {
      ...
      var sameTag = hasTag === false || utilities.isNullOrEmptySpace(child.tagName) === false && child.tagName.toLowerCase() === (tag + '').toLowerCase();
      var sameClassName = hasClassName === false || child.classList.contains(className);
  ...
  utilities.log = function (text, error) {
    // empty case
    if (utilities.isNullOrEmptySpace(text) || utilities.debugMode() === false && error !== true) {
      return '';
  ...
  utilities.btnLoading = function (element, on) {
    ...
    var loadingText = element.hasAttribute('data-loading-text') === true ? element.getAttribute('data-loading-text') : 'Working...';
    var originalText = element.hasAttribute('data-original-text') === true ? element.getAttribute('data-original-text') : 'Submit'; // set the text for the button and enable it
    ...
    element.innerHTML = '<i class="fa fa-refresh fa-spin fa-fw"></i>'; // add the button text to indicate what's happening

    element.innerHTML += loadingText; // disable the button while it is loading
  ...
  utilities.validAxiosResponse = function (response) {
    return utilities.isNullOrEmptySpace(response) === false && utilities.isNullOrEmptySpace(response.data) === false;
  }; // confirms whether the user is sure if they want to complete the given action

})(window.utilities = window.utilities || {}, window.jQuery, window, document, window.axios); // Down here is the code the defines the parameters used at the top of this
